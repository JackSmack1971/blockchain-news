...
app.post('/api/login/wallet', validateWalletAddress, authLimiter, async (req, res) => {
  try {
    const { signature } = req.body as { signature?: string };
    if (!signature) {
      return res.status(400).json({ error: 'Missing signature' });
    }
    const { walletAddress } = walletLoginSchema.parse({ walletAddress: req.body.walletAddress, signature });
